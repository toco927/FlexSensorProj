...
void loop() {
  ...
  for(int i = 0; i < 16; i++) {
    ...
    setDataVar(i,temp,controller_In);
    delay(10);//SWITCHING TIME FROM MULTIPLEXER
  }
...
void setDataVar(int i, int val1, controllerData &controllerStruct) {
  int val = val1;
  if (i > 15) {
  ...
  if (val1 > 4096) {
    Serial.println("ERROR! Data Value not valid (Driven over 5V logic limit: 4096)");
    Serial.println("Solution: Setting value to 14096, 1 being invalid flag!");
    val = 14096;
  }
